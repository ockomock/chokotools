...
class Codepad {
 public:
	...
	void set_codepad_lang(const string &);
...
void Codepad::to_lower(string &s)
{
	for (size_t i = 0; i < s.size(); i++)
		s.at(i) = tolower(s.at(i));
}

string Codepad::to_hex(char c)
{
	stringstream ss;
	ss << '%' << hex << int(c);
	return ss.str();
...
void Codepad::set_flags(const char *s)
{
	string str = s;
...
void Codepad::get_data(const char *s)
{
	string fn = s;
...
void Codepad::determine_lang(const string &s)
{
	string d;
...
void Codepad::get_content(const string &file)
{
	char c;
	ifstream f(file);
...
void Codepad::set_codepad_lang(const string &s)
{
	size_t i = 0;
